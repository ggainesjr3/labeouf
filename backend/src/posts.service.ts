...
export class PostsService {
  ...
  async getFeed(userId: number): Promise<any[]> {
    ...
    return this.withViewerState(combined.slice(0, 50), userId);
  }
  ...
  async likePost(userId: number, postId: number): Promise<{ liked: boolean; likeCount: number }> {
  ...
  private async withViewerState(posts: any[], userId: number): Promise<any[]> {
    ...
    const repostedIds = new Set(reposts.map(row => Number(row.postId)));
    const bookmarkedIds = new Set(bookmarks.map(row => Number(row.postId)));
